...
namespace MWUtilityScripts.UnityExtensions
{
    public static class MonoBehaviourExtensions
    {
            ...
            where TMember : Component
        {
            ...
            var (name, currentValue, type, setter) = memberInfo switch
            {
                ...
                _ => throw new ArgumentException(
                    $"Expression '{lambda}' did not return a field or property"
                ),
            };
            ...
            if (components.Length == 1)
            {
                ...
                EditorApplication.delayCall += () =>
                {
                    ...
                    setter(components[0]);
                    Debug.Log(
                        $"{nameof(AssignComponent)} successfully assigned {components[0].name} to {mb.name}"
                    );
        ...
        private static MemberInfo GetMemberInfo<TSource, TProperty>(
        ...
        )
        {
            if (memberLambda.Body is not MemberExpression member)
            {
                throw new ArgumentException(
                    $"Expression '{memberLambda}' refers to a method, not a property."
                );
            }
