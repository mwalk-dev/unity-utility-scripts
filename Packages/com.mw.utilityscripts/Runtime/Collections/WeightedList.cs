...
// Modified slightly to accept arbitrary PRNG implementations instead of only System.Random
...
namespace Runtime
{
    ...
    {
        public delegate int RandomInt(int max);

        private static readonly Random _random = new();
        private static readonly RandomInt _defaultRandom = (max) => _random.Next(max);

        /// <summary>
        /// Create a new WeightedList with an optional System.Random.
        /// </summary>
        /// <param name="rand"></param>
        public WeightedList(RandomInt rand)
        {
            _rand = rand ?? _defaultRandom;
        }

        /// <summary>
        /// Create a WeightedList with the provided items and an optional System.Random.
        /// </summary>
        public WeightedList(ICollection<WeightedListItem<T>> listItems, RandomInt rand = null)
        {
            _rand = rand ?? _defaultRandom;
        ...
        public T Next()
        {
            if (Count == 0)
                return default;
            int nextInt = _rand(Count);
            if (_areAllProbabilitiesIdentical)
                return _list[nextInt];
            int nextProbability = _rand(_totalWeight);
        ...
        private readonly RandomInt _rand;
